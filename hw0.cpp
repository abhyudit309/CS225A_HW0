...
const string robot_file_rprp = "./resources/rprpbot.urdf"; // urdf file location for RPRP robot
...
int main() {
	...
	auto robot_rprp = new Sai2Model::Sai2Model(robot_file_rprp, false); // loads RPRP robot also
	...
	*/
	...
	// ---------------------------  question 2-b -------------------------------
	ee_pos_in_link = Eigen::Vector3d(0.0, 0.0, 2.5);

	// ---------------------------  question 2-c -------------------------------
	// part i
	robot->_q << 0.0, 1.5, -M_PI/2;
	...
	robot->_q << 0.0, 1.5, -M_PI/2;
	robot->updateModel();
	file_2e_i << robot->_M(0,0) << "\t" << robot->_M(1,1) << "\t" << robot->_M(2,2) << "\n";
	int n_steps = 250;
	for(int i=0 ; i < n_steps ; i++)
	{
		robot->_q(2) = robot->_q(2) + M_PI/n_steps;
		robot->updateModel();
		file_2e_i << robot->_M(0,0) << "\t" << robot->_M(1,1) << "\t" << robot->_M(2,2) << "\n";
	}
	file_2e_i.close();
	...
	robot->_q << 0.0, 0.0, 0.0;
	robot->updateModel();
	file_2e_ii << robot->_M(0,0) << "\t" << robot->_M(1,1) << "\t" << robot->_M(2,2) << "\n";
	n_steps = 250;
	for(int i=0 ; i < n_steps ; i++)
	{
		robot->_q(1) = robot->_q(1) + 2.0/n_steps;
		robot->updateModel();
		file_2e_ii << robot->_M(0,0) << "\t" << robot->_M(1,1) << "\t" << robot->_M(2,2) << "\n";
	}
	file_2e_ii.close();
	...
	ofstream file_2g;
	file_2g.open("../../hw0/data_files/q2-g.txt");
	robot_rprp->_q << 0.0, 1.5, M_PI/4, 0.0;
	robot_rprp->updateModel();
	robot_rprp->gravityVector(grav_bis);
	file_2g << grav_bis.transpose() << "\n";
	n_steps = 250;
	for(int i=0 ; i < n_steps ; i++)
	{
		robot_rprp->_q(3) = robot_rprp->_q(3) + 2.0/n_steps;
		robot_rprp->updateModel();
		robot_rprp->gravityVector(grav_bis);
		file_2g << grav_bis.transpose() << "\n";
	}
